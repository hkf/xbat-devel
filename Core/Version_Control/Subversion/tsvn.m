...
% TODO: we need to look at this logic, which tests to perform

if exist(p, 'dir') && ~is_working_copy(p)
...
cli_pre = ['"', TSVN_CLIENT, '" ', interact, ' /command:'];
...
% db_disp(' '); disp(cli_str);

[status, result] = system(cli_str);

if status == -1
	disp(' ');
	disp('Failed on command:');
	disp(['  ', cli_str]);
	disp('Error message:');
	if ~isempty(result)
		disp(['  ', result]);
