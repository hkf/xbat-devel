function out = annotation_interface(mode,varargin)

% annotation_interface - simple code and notes annotation
% ----------------------------------------------------
%
%  annotation = annotation_interface('create')
% description = annotation_interface('describe');
%
%         log = annotation_interface('batch',log,ix,annotation)
%       event = annotation_interface('batch',event,annotation)
%
%        flag = annotation_interface('selection',h)
%             = annotation_interface('events',h,m,ix)
%
%           g = annotation_interface('menu',h,m,ix)
%             = annotation_interface('display',h,m,ix)
%
% Input:
% ------
...
%--------------------------------------------------------
% LOG SELECTION WITH ANNOTATION (ONLY HAPPENS IN SOUND BROWSER)
%--------------------------------------------------------
...
%--------------------------------------------------------
% BATCH OPERATIONS (THESE ARE MEANT FOR THE COMMAND LINE)
%--------------------------------------------------------
...
%--------------------------------------------------------
% DISPLAY ANNOTATION INFORMATION IN CONTEXTUAL MENU OR GRAPHICALLY
%--------------------------------------------------------
...
% 			mg = menu_group(g,'',{description.name,['Edit ' description.name ' ...']});
	...
	if (isempty(value{ixf}))
		...
		switch (class(value{ixf}))	
			...
			case ('char')
				...
				% truncate long strings to length ~32 characters
...
%--
% display code string top center of event as highlighted text
%--
...
%--
% add mode to the input of the view function and to the annotate menu
%--
