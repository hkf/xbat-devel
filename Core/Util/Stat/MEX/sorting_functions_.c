...
double opt_med3_double(double *p, int n);
...
double opt_med5_double(double *p, int n);
...
double opt_med7_double(double *p, int n);
...
double opt_med9_double(double *p, int n);
...
double quick_select_double (double p[], int n);
...

{
    ...
    for (;;) {
		...
		// Find med of l, mid and h items; swap into position l

		mid = (l + h) / 2;

		if (p[mid] > p[h]) {
			ELEM_SWAP_D(p[mid],p[h]);
		}
		if (p[l] > p[h]) {
			ELEM_SWAP_D(p[l],p[h]);
		}
		...
		if (p[mid] > p[l]) {
			ELEM_SWAP_D(p[mid],p[l]);
		}

		// Swap l item (now in position mid) into position (l + 1)

		ELEM_SWAP_D(p[mid],p[l + 1]) ;

		// Nibble from each end towards mid, swapping items when stuck
		...
		for (;;) {
			...
			ELEM_SWAP_D(p[ll],p[hh]);
		
		}

		// Swap mid item (in position l) back into correct position

		ELEM_SWAP_D(p[l],p[hh]);
