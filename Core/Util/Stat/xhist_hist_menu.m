function xhist_hist_menu(h,str,flag)

% xhist_hist_menu - histogram viewing tools menu
% ----------------------------------------
%
% xhist_hist_menu(h,str,flag)
%
% Input:
% ------
...
[LINESTYLE,SEP_LINESTYLE] = linestyle_to_str('','strict');
...
case ('Initialize')
	...
	tmp = menu_group(h,'xhist_hist_menu',L,S,A);
	...
	data.browser.histogram_options = ...
		menu_group(get_menu(h,'Histogram Options'),'xhist_hist_menu',L,S);	
	...
	tmp = menu_group(get_menu(h,'Number of Bins'),'xhist_hist_menu',L,S,A);
	...
	tmp = ...
		...
		'xhist_hist_menu',COLOR,SEP_COLOR);
	...
	tmp = ...
		...
		'xhist_hist_menu',LINESTYLE,SEP_LINESTYLE);
	...
	data.browser.kernel_options = ...
		menu_group(get_menu(h,'Kernel Options'),'xhist_hist_menu',L,S,A);
	...
	tmp = menu_group(get_menu(h,'Length'),'xhist_hist_menu',L,S,A);
	...
	tmp = menu_group(get_menu(h,'Kernel'),'xhist_hist_menu',L,S,A);
	...
	data.browser.fit_options = ...
		menu_group(get_menu(h,'Fit Model Options'),'xhist_hist_menu',L,S,A);
	...
	if (data.browser.kernel.view)
		xhist_hist_menu(h,'Kernel Display');
...
case ('Histogram')
	...
	xhist_hist_menu(h,'Histogram Display');
...
case ('Histogram Display')
	...
	if (hist.view)
		...
		if (iscell(hist.counts))
			...
			%--
			% remove previously displayed histograms
			%--
			...
			for k = 1:length(hist.counts)
				...
				tmp = plot(hist.centers{k},nh, ...
					'linestyle',linestyle_to_str(hist.linestyle,'strict'), ...
		...
		if (iscell(hist.counts))
			...
			for k = 1:length(hist.counts)
				...
				tmp = plot(hist.centers{k},kh, ...
					'linestyle',linestyle_to_str(kernel.linestyle,'strict'), ...
		...
		if (iscell(hist.counts))
			...
			for k = 1:length(hist.counts)
				...
				tmp = plot(hist.centers{k},fh, ...
					'linestyle',linestyle_to_str(fit.linestyle,'strict'), ...
...
case (LINESTYLE)
	...
	case ('Histogram Options')
		...
		if (data.browser.hist.view)
			...
			if (~isempty(tmp))
				set(tmp,'marker','none');
				set(tmp,'linestyle',linestyle_to_str(str,'strict'));
			end
		end
