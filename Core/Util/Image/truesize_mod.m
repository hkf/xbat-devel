function truesize_mod(varargin)
...
%   $Revision: 1.1.8.1 $  $Date: 2004/08/10 01:50:40 $
...
elseif (isempty(colorbarHandle))
    ...
    if (length(colorbarHandle) > 1)
        ...
        colorbarAxes = ancestor(colorbarHandle, 'axes');
...
caxHandle = ancestor(colorbarHandle, 'axes');
figHandle = ancestor(axHandle, 'figure');
figUnits = get(figHandle, 'Units');
