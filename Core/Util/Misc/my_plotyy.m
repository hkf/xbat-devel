function [axout,h1,h2]=my_plotyy(varargin)

% my_plotyy - modified version of plotyy
% --------------------------------------
%
% NOTE: look at 'plotyy' help
...
% NOTE: this function is too slow as is to use in active detection display
...
%
%   See also PLOT, @
%   Type "doc my_plotyy" for more information.
...
% NOTE: first see if we have a vector of associated PLOTYY axes
...
%--
% Find bestscale that produces the same number of y-ticks for both the left and the right.
%--
...
if ~isempty(low)
	...
	%--
	% Set tick labels if axis ticks aren't at decade boundaries when in log mode
	%--
...
%--
% Filter out the cases where power of ten's don't match
%--
