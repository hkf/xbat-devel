...
if ( ...
	~strcmp(CreateMode,'non-modal') & ...
	~strcmp(CreateMode,'modal') & ...
...
if ~( ...
	strcmp(IconString,'none') | ...
	strcmp(IconString,'help') | ...
	strcmp(IconString,'warn') | ...
	strcmp(IconString,'error') | ...
...
%-----------------------------------------------
% SET FONT SIZE USING PALETTE INFORMATION
%-----------------------------------------------
...
try
	...
	Font.FontSize = 1.2 * tmp.fontsize; % display in the same size as headers
...
if isempty(gcbf)
	...
	%--
	% compute figure size using screensize and figure size
	%--
	...
	%--
	% compute figure size using callback figure and figure size
	%--

	DefFigPos(1:2) = [ ...
		(GCBFPos(1) + GCBFPos(3) / 2) - FigWidth / 2, ...
